...
public class ConfigurationMoD {
  ...
  public static void loadConfig() {
    ...
    MapDisapearAfterTeleport = KingdomRegions.config.getBoolean("MapDisapearAfterTeleport", CATEGORY_2, false, "Enable this to let the Map dispper after use.");
    useMapCooldown = KingdomRegions.config.getBoolean("useMapCooldown", CATEGORY_2, true, "Enable this to use the Cooldownsystem for Teleports.");
    canTeleportToVillages = KingdomRegions.config.getBoolean("canTeleportToVillages", CATEGORY_2, true, "Enable it to let the Player use Runestones.");
    TeleportCooldown = KingdomRegions.config.get(CATEGORY_2, "Teleportcooldown in secounds", 10).getInt() * 40;
    ...
    HTML_COLOR = KingdomRegions.config.get(CATEGORY_3, "Colorcode for the Display Text ", 16119039).getInt();
    DisplayTime = KingdomRegions.config.get(CATEGORY_3, "the Time in Secounds which the Text is shown", 4).getInt() * 40;
    ...
    TownNames = KingdomRegions.config.getStringList("TownNames", "List", new String[] {
          "Demonsummit", "Fallhold", "Ebonholde", "Lakeville", "Nevermoor", "Goldcrest", "Highbury", "Sleetdale", "Brittletide", "Sleekborn", 
          "Swiftspell", "Silkbay", "Doghall", "Glimmerharbor", "Muddrift", "Ghostgrasp", "Roseharbor", "Sleetstar", "Deadwallow", "Shimmervault", 
          "Mageshore", "Mistville", "Snowgrasp", "Feardenn", "Sleekborn", "Mistcoast", "Baygulch", "Oxkeep", "Stagcrest", "Whitspire", 
          "Sunfalls", "Mutehaven", "Northburgh", "Newgarde", "Quickhold", "Stillfall", "Whitcross", "Silveracre", "Whiteview", "Grimefair", 
          "Nightburgh", "Kilgrove", "Highfield", "Lakeharbor", "Coldhallow", "Swampshield", "Deepstrand", "Whiteshire", "Whitereach", "Rockhand", 
          "Deadstorm", "Sleetholde", "Quickwind", "Nightfield", "Scorchwood", "Rosehaven", "Southbay" }, "Standard Names for Villages.");
  ...
  public static void addPoint(int[] intlist, String string) {
    Size = KingdomRegions.config.get(CATEGORY_2, "CurrentPoints", 0).getInt();
    Size++;
    KingdomRegions.config.get(CATEGORY_2, "cords_" + Size, intlist).getIntList();
    KingdomRegions.config.get(CATEGORY_2, "cords_" + Size + "name", string).getString();
  ...
  public static void checkPlayerPos(EntityPlayer player) {
    ...
    for (int i = 1; i <= Size; i++) {
      ...
      if (player.getDistance(ValueList[0], ValueList[1], ValueList[2]) <= ValueList[3])
        PacketDispatcher.sendTo((IMessage)new sendVillagePacket(KingdomRegions.config.get(CATEGORY_2, "cords_" + i + "name", "none").getString(), new ChunkCoordinates(ValueList[0], ValueList[1], ValueList[2]), ValueList[3]), (EntityPlayerMP)player);
    } 
    if (KingdomRegions.config.hasChanged())
      KingdomRegions.config.save();
  }
